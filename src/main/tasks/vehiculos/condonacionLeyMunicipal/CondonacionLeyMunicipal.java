...
public class CondonacionLeyMunicipal extends Generator {
    protected String operacion;
    protected String identificador;
    protected String gestionInicio;
    protected String gestionFin;
    protected String concepto;
    protected String porcentajeCondonacion;

    public CondonacionLeyMunicipal(){
        super();
        this.accessExcel = new AccessExcel(ConstantsVEH.GENERATOR_DATA_FILE, ConstantsVEH.CONDONACION_LEY_MUNICIPAL_DATA_SHEET);
    ...
    public void start() {
        ...
        LoadModule.whichIs(this.driverApp, ConstantsVEH.CONDONACION_LEY_MUNICIPAL_MODULE);
        ...
        if(Verify.isReady(this.driverApp, test.get(i), ReceiveDocumentationUI.ttlRecepcionarDocumentacion)){
            ...
            if (Verify.isReady(this.driverApp, test.get(i), DetalleGestionesUI.ttlDetalleGestiones)){
                if(this.concepto.equals("TODOS")){
                    boolean somethingWasSelected = DetalleGestiones.selectAll(this.driverApp, this.operacion);
                    if (somethingWasSelected){
                        ...
                        if (this.operacion.equals("REGISTRAR")){
                            RegistrarCondonacion.now(this.driverApp);
                        }else if (this.operacion.equals("ANULAR")){
                            AnularCondonacion.now(this.driverApp, this.operacion, test.get(i));
                        }
                        if (Verify.isReady(this.driverApp, test.get(i), DetalleGestionesUI.ttlDetalleGestiones)){
                            Click.on(this.driverApp, DetalleGestionesUI.btnAceptar);
                            Verify.isLoading(this.driverApp, 300);
